		...
		ImgurOnMessage: {
			...
			command: async function( str, event, prefix = true ) {
				...
				if( image !== null ) {
					...
					const data   = await self.commands.fetchImage.command( parsed_image );
		...
		fetchImage: {
			command: async function( id ) {
				const result = await imgurapi.getInfo( id ).then( ( res ) => {
					...
					let parsedTime   = core.utils.parseUnixTime( img.datetime );
					parsedTime       = core.utils.formatToFancyTime( parsedTime, dateFormat );
